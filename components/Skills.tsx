    ...
    <section className="flex flex-col justify-start items-center py-[72px] bg-primary-light dark:bg-primary-darkDefault w-full">
      ...
      <div className="flex flex-wrap justify-center pt-[48px] gap-[41px] px-[85px]">
        {skills.map((skill, index) => (
          <div
            key={index}
            className={`m-4 bg-[#F3F8FF] hover:bg-[#FFFFFF] hover:drop-shadow-2xl transform transition-transform duration-300 hover:scale-116 dark:bg-[#192333] w-[100px] h-[100px] rounded-full flex items-center justify-center`}
          >
            <Image
              src={skill.imgURL}
              alt={`Skill ${index}`}
