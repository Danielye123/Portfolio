...
const Skills = () => {
  return (
    <section className="flex flex-col justify-start items-center md:py-[72px] md:px-[85px] py-[40px] bg-primary-light dark:bg-primary-darkDefault w-full">
<h1 className="text-primary-darkDefault dark:text-primary-light font-poppins font-bold text-[36px] md:text-[48px] leading-[41px] md:leading-[55px] tracking-[-0.01em] pb-[40px] max-w-[390px] ">
      ...
      <div className="grid grid-cols-4 gap-x-[2.5rem] gap-y-8 md:flex md:flex-wrap md:justify-center lg:gap-[4rem] 
        ...
        {skills.map((skill, index) => (
          <div
            key={index}
            className={`m-4 bg-[#F3F8FF] hover:bg-[#FFFFFF] hover:drop-shadow-2xl dark:hover:drop-shadow-2xl transform transition-transform hover:scale-110 dark:bg-[#192333] w-[54px] h-[52px] md:w-[100px] md:h-[100px] rounded-full flex items-center justify-center
            grayscale hover:grayscale-0 ease-in
            ${index === 0 && 'md:order-1'}`}
          >
